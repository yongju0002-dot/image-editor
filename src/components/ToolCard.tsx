...
export function ToolCard({ tool }: { tool: Tool }) {
  ...
  if (!tool.available) {
    return (
      <div className="rounded-2xl border border-dashed border-zinc-200 bg-zinc-50/60 p-7 opacity-60 dark:border-zinc-800 dark:bg-zinc-900/40">
        <div
          className={`flex h-16 w-16 items-center justify-center rounded-xl grayscale ${meta.iconBg} ${meta.iconBgDark}`}
        >
          <Icon
            className={`h-8 w-8 ${meta.iconText} ${meta.iconTextDark}`}
            strokeWidth={1.75}
          />
        </div>
  ...
  return (
    ...
    >
      <div
        className={`flex h-16 w-16 items-center justify-center rounded-xl transition-transform duration-200 group-hover:scale-105 ${meta.iconBg} ${meta.iconBgDark}`}
      >
        <Icon
          className={`h-8 w-8 ${meta.iconText} ${meta.iconTextDark}`}
          strokeWidth={1.75}
